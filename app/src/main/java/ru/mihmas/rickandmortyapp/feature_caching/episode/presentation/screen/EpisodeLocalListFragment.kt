...
import ru.mihmas.rickandmortyapp.feature_caching.episode.presentation.recyclerview.CachedEpisodeAdapter
import ru.mihmas.rickandmortyapp.feature_caching.episode.presentation.viewmodel.CachedEpisodeViewModel
import ru.mihmas.rickandmortyapp.feature_caching.episode.presentation.viewmodel.CachedEpisodeViewModelFactory
import ru.mihmas.rickandmortyapp.feature_list.episode.presentation.screen.EpisodeRemoteListFragment
import javax.inject.Inject

class EpisodeLocalListFragment : Fragment() {
    @Inject
    lateinit var viewModelFactory: CachedEpisodeViewModelFactory
    @Inject
    lateinit var episodeAdapter: CachedEpisodeAdapter
    private val component by lazy {
        (requireActivity().application as App).component
    }
    private val viewModel by lazy {
        ViewModelProvider(requireActivity(), viewModelFactory)[CachedEpisodeViewModel::class.java]
    }
        ...
        searchView.apply {
            ...
            setOnQueryTextListener(object : SearchView.OnQueryTextListener {
                override fun onQueryTextSubmit(name: String): Boolean {
                    viewModel.findEpisode(name = name)
                    return true
                }
        ...
        cacheView.setOnMenuItemClickListener {
            parentFragmentManager.beginTransaction()
                .replace(
                    ...
                    EpisodeRemoteListFragment.getFragment()
                ).commit()
            true
        }
    }
    ...
    private fun openLocationFilterDialog() {
        CachedEpisodeFilterDialog().show(
    ...
    companion object {
        const val EPISODE_DIALOG_BUNDLE = "LOCAL_EPISODE_DIALOG_BUNDLE"
        const val FILTER_EPISODE = "FILTER_EPISODE"
        private const val DEFAULT_VALUE = "DEFAULT_VALUE"
        private const val FILTER_TAG = "Local episode filter dialog"

        fun getFragment(): EpisodeLocalListFragment = EpisodeLocalListFragment()
    }
}